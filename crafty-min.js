...
(function (i, f, h) {
    ...
    i.c("2D", {
        ...
        _rotate: function (y) {
            ...
            var E = z.x + (this._x - z.x) * w + (this._y - z.y) * A,
                r = z.y - (this._x - z.x) * A + (this._y - z.y) * w,
                D = z.x + (this._x + this._w - z.x) * w + (this._y - z.y) * A,
                p = z.y - (this._x + this._w - z.x) * A + (this._y - z.y) * w,
                C = z.x + (this._x + this._w - z.x) * w + (this._y + this._h - z.y) * A,
                n = z.y - (this._x + this._w - z.x) * A + (this._y + this._h - z.y) * w,
                B = z.x + (this._x - z.x) * w + (this._y + this._h - z.y) * A,
                e = z.y - (this._x - z.x) * A + (this._y + this._h - z.y) * w,
                u = Math.floor(Math.min(E, D, C, B)),
                s = Math.floor(Math.min(r, p, n, e)),
                t = Math.ceil(Math.max(E, D, C, B)),
                q = Math.ceil(Math.max(r, p, n, e));
            this._mbr = {
                _x: u,
                _y: s,
                _w: t - u,
    ...
    i.c("Collision", {
        ...
        hit: function (u) {
            var n = this._mbr || this,
                r = i.map.search(n, false),
                s = 0,
                p = r.length,
                y = {}, e, q, t, x, v = ("map" in this && "containsPoint" in this.map),
                o = [];
            if (!p) {
                return false
            }
    ...
    i.c("DOM", {
        ...
        draw: function () {
            var o = this._element.style,
                r = this.__coord || [0, 0, 0, 0],
                q = {
                    x: r[0],
                    y: r[1]
                }, p = i.support.prefix,
        ...
        DOM: {
            ...
            inner: function (q) {
                var p = q.getBoundingClientRect(),
                    e = p.left + (f.pageXOffset ? f.pageXOffset : h.body.scrollTop),
                    r = p.top + (f.pageYOffset ? f.pageYOffset : h.body.scrollLeft),
                    o, n;
        ...
        viewport: {
            ...
            init: function (e, o) {
                ...
                i.addEvent(this, f, "resize", function () {
                    i.DOM.window.init();
                    var t = i.DOM.window.width,
                        u = i.DOM.window.height,
                        v;
                ...
                if (i.mobile) {
                    ...
                    p.left = "0px";
                    p.top = "0px";
                    var r = h.createElement("meta"),
                        n = h.getElementsByTagName("HEAD")[0];
    ...
    (function b() {
        var p = i.support,
            ...
            n = /(webkit)[ \/]([\w.]+)/.exec(o) || /(o)pera(?:.*version)?[ \/]([\w.]+)/.exec(o) || /(ms)ie ([\w.]+)/.exec(o) || /(moz)illa(?:.*? rv:([\w.]+))?/.exec(o) || [],
            e = /iPad|iPod|iPhone|Android|webOS/i.exec(o);
        if (e) {
            i.mobile = e[0]
        }
    ...
    i.c("Canvas", {
        ...
        draw: function (v, s, q, t, n) {
            ...
            var r = {
                ...
                _w: (t || this._w),
                _h: (n || this._h)
            }, e = v || i.canvas.context,
                o = this.__coord || [0, 0, 0, 0],
                p = {
                    x: o[0] + (s || 0),
                    y: o[1] + (q || 0),
                    w: t || o[2],
    ...
    i.extend({
        ...
        mouseDispatch: function (u) {
            ...
            var v = -1,
                o, n, r = 0,
                p, B = i.DOM.translate(u.clientX, u.clientY),
                C, A, D = {}, s = u.target ? u.target : u.srcElement,
                z = u.type;
            if (z === "touchstart") {
                z = "mousedown"
            } else {
    ...
    i.DrawManager = (function () {
        ...
        return {
            ...
            drawAll: function (u) {
                var u = u || i.viewport.rect(),
                    v, t = 0,
                    s, r = i.canvas.context,
                    w;
                v = i.map.search(u);
                s = v.length;
            ...
            draw: function e() {
                ...
                for (A = 0, t = C.length; A < t; ++A) {
                    ...
                    E = z.obj;
                    var s = E._mbr || E,
                        H = (G._x - s._x <= 0) ? 0 : ~~(G._x - s._x),
                        F = (G._y - s._y < 0) ? 0 : ~~(G._y - s._y),
                        I = ~~Math.min(s._w - H, G._w - (s._x - G._x), G._w, s._w),
                        B = ~~Math.min(s._h - F, G._h - (s._y - G._y), G._h, s._h);
                    if (B === 0 || I === 0) {
                        continue
                    }
                    J.save();
